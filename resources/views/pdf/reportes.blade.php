...
<body>
    ...
    <div class="resumen">
        <div>
            ...
            <div class="value" style="color:#059669">${{ format_amount($totalIngresos, 2, ',', '.') }}</div>
        </div>
        <div>
            <div class="label">Total Egresos</div>
            <div class="value" style="color:#dc2626">${{ format_amount($totalEgresos, 2, ',', '.') }}</div>
        </div>
        <div>
            <div class="label">Saldo</div>
            <div class="value" style="color:{{ $saldo >= 0 ? '#059669' : '#dc2626' }}">${{ format_amount($saldo, 2, ',', '.') }}</div>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td align="right" class="{{ $m->tipo }}">${{ format_amount($m->monto, 2, ',', '.') }}</td>
            </tr>
            @empty
